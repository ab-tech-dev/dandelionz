...
class AuthenticationService:
    ...
    def login(email, password, request_meta = None, request = None):
        ...
        try:
            ...
            try:
                ...
                if not user_exists:
                    logger.warning(f"Logn attempt for non-existent email: {email}")
            ...
            # Record login with the device information for audit trail
            try:
    ...
    def refresh_token(refresh_token):
        """Refresh and authentication token"""
    ...
    def validate_token(token, user):
        ...
        if not is_valid or user_id != user.id:
            ...
            return False, {
                "success": False,
                "error": "Token valiiidation failed"
            }, 401
    ...
    def logout(user, refresh_token = None):
        """Handle user logout invalidating token as needed"""
        if refresh_token:
            try:
